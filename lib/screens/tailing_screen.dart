...
class TailingScreen extends StatefulWidget {
  const TailingScreen({Key? key}) : super(key: key);

  @override
  State<TailingScreen> createState() => _TailingScreenState();
}

class _TailingScreenState extends State<TailingScreen> {
  DateTime? startDate;
  DateTime? endDate;
  String selectedTimeRange = 'روزانه';
  bool isLoading = false;
  List<TailingData> tailingData = [];
  ...
  Future<void> _loadTailingData() async {
    ...
    try {
      ...
      final convertedData = _convertToTailingData(productionData);
      final stats = _calculateSummaryStats(convertedData);

      setState(() {
        tailingData = convertedData;
        summaryStats = stats;
        isLoading = false;
      });
    } catch (e) {
    ...
    switch (selectedTimeRange) {
      case 'روزانه':
        ...
        dayGroups.forEach((day, list) {
          final total = list.fold<double>(0, (sum, e) => sum + e.waste);
          result.add(TailingData(
            label: day.toString(),
            actualTailing: total,
            plannedTailing: PlanningCalculationService.getPlanByTimeRange(
  ...
  Map<String, double> _calculateSummaryStats(List<TailingData> tailingData) {
    if (tailingData.isEmpty) return {};
    ...
    for (final data in tailingData) {
      totalActual += data.actualTailing;
      totalPlanned += data.plannedTailing;

      if (data.actualTailing > maxDaily) maxDaily = data.actualTailing;
      if (data.actualTailing < minDaily) minDaily = data.actualTailing;
    }
  ...
  Future<void> _printReport() async {
    ...
    try {
      ...
      final actualData = tailingData.asMap().entries.map((entry) {
        return ChartPoint(
            entry.key.toDouble(), entry.value.actualTailing / 1000);
      }).toList();

      final plannedData = tailingData.asMap().entries.map((entry) {
        return ChartPoint(
            entry.key.toDouble(), entry.value.plannedTailing / 1000);
      }).toList();

      // تبدیل داده‌های جدول
      final pdfTailingData = tailingData.map((data) {
        return PdfTailingData(
          label: data.label,
          actualTailing: data.actualTailing,
          plannedTailing: data.plannedTailing,
          shift: data.shift,
        );
      }).toList();

      // تولید PDF
      final pdfFile = await PdfReportService.generateTailingReport(
        tailingData: pdfTailingData,
        startDate: startDate!,
        endDate: endDate!,
        ...
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) => PdfPreviewScreen(
              pdfFile: pdfFile,
              title: 'پرینت گزارش باطله تولیدی',
            ),
          ),
        );
      }
  ...
  Widget _buildChart() {
    ...
    double maxY = tailingData
        .map((d) => d.actualTailing)
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            'مقایسه باطله محقق شده و برنامه ($yUnit)',
            ...
            child: Stack(
              children: [
                BarChart(
                  BarChartData(
                    barGroups: [
                      for (int i = 0; i < tailingData.length; i++)
                        BarChartGroupData(
                          x: i,
                          barRods: [
                            BarChartRodData(
                              toY: tailingData[i].actualTailing / yDiv,
                              color: Colors.red,
                              width: 16,
                    ...
                    titlesData: FlTitlesData(
                      ...
                      bottomTitles: AxisTitles(
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, meta) {
                            if (value.toInt() >= 0 &&
                                value.toInt() < tailingData.length) {
                              return Padding(
                                padding: const EdgeInsets.only(top: 8.0),
                                child: Text(
                                  tailingData[value.toInt()].label,
                                  style: const TextStyle(fontSize: 10),
                                  textAlign: TextAlign.center,
                                ),
                              );
                        ...
                        lineBarsData: [
                          LineChartBarData(
                            spots: [
                              for (int i = 0; i < tailingData.length; i++)
                                FlSpot(i.toDouble(),
                                    tailingData[i].plannedTailing / yDiv),
                            ],
                            isCurved: true,
                            color: Colors.blue,
                            barWidth: 2,
          ...
          Row(
            ...
            children: [
              _buildLegendItem('باطله محقق شده', Colors.red),
              const SizedBox(width: 24),
              _buildLegendItem('برنامه سالانه', Colors.blue),
            ],
          ),
        ],
      ),
  ...
  Widget _buildDataTable() {
    ...
    String tableTitle = 'جزئیات باطله تولیدی';
    switch (selectedTimeRange) {
      case 'روزانه':
        tableTitle = 'جزئیات باطله تولیدی - روزانه';
        break;
      case 'هفتگی':
        tableTitle = 'جزئیات باطله تولیدی - هفتگی';
        break;
      case 'ماهانه':
        tableTitle = 'جزئیات باطله تولیدی - ماهانه';
        break;
      case 'شیفتی':
        tableTitle = 'جزئیات باطله تولیدی - شیفتی';
        break;
    }
          ...
          SingleChildScrollView(
            ...
            child: DataTable(
              columns: const [
                ...
                DataColumn(label: Text('باطله محقق شده (تن)')),
                DataColumn(label: Text('باطله برنامه‌ریزی شده (تن)')),
                DataColumn(label: Text('انحراف از برنامه')),
              ],
              rows: tailingData.map((data) {
                final deviation = data.actualTailing - data.plannedTailing;
                ...
                return DataRow(
                  cells: [
                    ...
                    DataCell(Text(data.actualTailing.toStringAsFixed(1))),
                    DataCell(Text(data.plannedTailing.toStringAsFixed(1))),
                    DataCell(
                      Text(
                        data.plannedTailing > 0
                            ? '${(((data.actualTailing - data.plannedTailing) / data.plannedTailing) * 100).toStringAsFixed(1)}%'
                            : '-',
                        style: TextStyle(
                          color: data.plannedTailing > 0 &&
                                  data.actualTailing - data.plannedTailing >= 0
                              ? Colors.green
                              : Colors.red,
                          fontWeight: FontWeight.bold,
                        ),
...
class TailingData {
  final String label;
  double actualTailing;
  double plannedTailing;
  final String shift;
